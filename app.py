...
PDF_JOKBO_DATA = [
    ...
    {"과목": "전기이론", "문제": "규격이 같은 축전지 두 개를 병렬로 연결하였다. 설명 중 옳은 것은?", "보기": ["1) 전압과 용량 모두 2배가 된다.", "2) 전압은 2배가 되고 용량은 변하지 않는다.", "3) 전압은 변하지 않고 용량은 2배가 된다.", "4) 전압과 용량 모두 변하지 않는다."], "정답": "3", "해설": "병렬 연결 시 전압은 일정하고 전체 용량(Ah)은 개수에 비례하여 늘어낭니다."},
    ...
    {"과목": "전기설비", "문제": "전등 한 개를 두 개소에서 점멸하고자 할 때 3로 스위치는 최소 몇 개가 필요한가?", "보기": ["1) 1개", "2) 2개", "3) 3개", "4) 4개"], "정답": "2", "해설": "2개소 점멸 제어 회로를 구성하기 위해서는 3로 스위 2개가 양쪽에 필요합니다."}
]

# ⭐ [추가 기능] AI 텍스트 내 수식 공백 깨짐 현상을 원천 방지하는 보안 정제 함수
def clean_latex_string(text):
    if not text:
        return ""
    # 구글이 간혹 출력하는 잘못된 백슬래시 이중 이스케이프 정제
    text = text.replace("\\\\frac", "\\frac").replace("\\\\sqrt", "\\sqrt")
    # $ 기호 바로 뒤나 앞에 공백이 있으면 Streamlit 수식 엔진이 인지하지 못하므로 자동 공백 압축
...
def generate_ai_batch(api_key, subject, count):
    ...
    system_prompt = (
        ...
        "2. 분수는 반드시 $\\frac{분자}{분모}$ 형태로 작성해라. 기호 내부에 과도한 여백은 제거해라.\n"
    ...
    safety_settings = [
        types.SafetySetting(category=types.HarmCategory.HARM_CATEGORY_HEE_SPEECH, threshold=types.HarmBlockThreshold.BLOCK_NONE),
...
# 🛠️ [PDF 족보 20문제 + AI 변형 40문제] 최종 시험지 믹스 빌더
