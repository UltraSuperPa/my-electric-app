...
# 1. 📌 김경욱 님이 업로드해주신 [기적의 200문제] 엄선 핵심 족보 데이터 보관함
PDF_JOKBO_DATA = [
    ...
    {"과목": "전기기기", "문제": "동기 발전기의 변렬 운전에서 같지 않아도 되는 것은?", "보기": ["1) 기전력의 크기", "2) 기전력의 위상", "3) 발전기의 용량", "4) 기전력의 주파수"], "정답": "3", "해설": "동기발전기 병렬운전 조건은 크기, 위상, 주파수, 파형, 상회전 방향이 같아야 하며, 용량은 달라도 됩니다."},
...
def generate_60_exams(api_key):
    # 1. PDF 족보에서 20문제를 고정으로 섞어서 추출
    jokbo_sample = random.sample(PDF_JOKBO_DATA, min(20, len(PDF_JOKBO_DATA)))
    
    # 2. 구글 제한(Rate Limit)을 우회하기 위해 총 3번만 AI를 호출하여 나머지 40문제 확보
    # 과목 밸런스 균등 배정 (14문제 + 13문제 + 13문제 = 총 40문제)
